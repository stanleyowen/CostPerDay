...
struct DaysOwnedTests {
    ...
    func dayOfPurchaseIsOne() {
        #expect(makeItem(boughtDaysAgo: 0).daysOwned(now: now) == 1)
    }

    @Test("Each elapsed day adds one")
    func countsElapsedDays() {
        #expect(makeItem(boughtDaysAgo: 1).daysOwned(now: now) == 2)
        #expect(makeItem(boughtDaysAgo: 364).daysOwned(now: now) == 365)
    }
    ...
    func retirementStopsTheClock() {
        let item = makeItem(boughtDaysAgo: 400)
    ...
    func nonsenseRetirementIsClamped() {
        let item = makeItem(boughtDaysAgo: 100)
...
struct CostPerDayTests {
    ...
    func actualCost() {
        let item = makeItem(price: 1000, boughtDaysAgo: 99) // 100 days owned
        #expect(abs(item.actualCostPerDay(now: now) - 10) < 0.0001)
    }

    @Test("Planned cost is price divided by the expected lifetime")
    func plannedCost() {
        let item = makeItem(price: 1096.5, lifetimeMonths: 36) // 1096 planned days
    ...
    func costFallsOverTime() {
        let young = makeItem(price: 1000, boughtDaysAgo: 10)
        let old = makeItem(price: 1000, boughtDaysAgo: 1000)
    ...
    func resaleReducesCost() {
        let item = makeItem(price: 1000, boughtDaysAgo: 99)
    ...
    func modeSelection() {
        let item = makeItem(price: 1000, boughtDaysAgo: 99, lifetimeMonths: 36)
...
struct PaidOffTests {
    ...
    func notYetPaidOff() {
        let item = makeItem(lifetimeMonths: 1) // 30 planned days
    ...
    func paidOffOnBoundary() {
        let item = makeItem(lifetimeMonths: 1)
    ...
    func remainingIsFloored() {
        let item = makeItem(boughtDaysAgo: 3650, lifetimeMonths: 1)
        #expect(item.daysRemaining(now: now) == 0)
    }
    ...
    func lifetimeIsCapped() {
        ...
        #expect(item.plannedDays <= Int(Double(Item.maxLifetimeMonths) * 30.4375) + 1)
...
struct CurrencyTests {
    @Test("An item in the base currency needs no conversion")
    func baseCurrencyIsUnchanged() {
        let item = makeItem(price: 500, currency: "USD", rate: 1)
        #expect(item.priceInBase == 500)
    }

    @Test("A foreign price converts at the rate locked at purchase")
    func foreignConverts() {
        let item = makeItem(price: 30_000, currency: "TWD", rate: 0.031)
    ...
    func invalidRateFallsBack() {
        for bad in [0.0, -2.0, Double.nan, Double.infinity] {
            let item = makeItem(price: 100, currency: "JPY", rate: bad)
    ...
    func resaleUsesItsOwnRate() {
        let item = makeItem(price: 1000, currency: "EUR", rate: 1.10)
    ...
    func rebaseScalesRates() {
        let usd = makeItem(price: 100, currency: "USD", rate: 1)
        let twd = makeItem(price: 3000, currency: "TWD", rate: 0.031)
        Item.rebase([usd, twd], by: 32) // 1 USD = 32 TWD
    ...
    func rebaseRejectsBadFactor() {
        let item = makeItem(price: 100, rate: 1)
        for bad in [0.0, -1.0, Double.nan] {
            Item.rebase([item], by: bad)
...
struct ValidationTests {
    private func issues(_ item: Item, base: String = "USD") -> [ItemValidation.Issue] {
        ItemValidation.issues(for: item, baseCurrency: base, now: now)
    }

    @Test("A well-formed gadget has no complaints")
    func validItemPasses() {
        #expect(issues(makeItem()).isEmpty)
    }
    ...
    func largeNativePriceInLowValueCurrencyPasses() {
        ...
        let item = makeItem(price: 17_249_000, currency: "IDR", rate: 0.000061)
    ...
    func genuinelyAbsurdPriceIsRejected() {
        let item = makeItem(price: 999_999_999_999, currency: "IDR", rate: 0.000061)
        #expect(issues(item, base: "USD").contains { $0.field == .price })
    }
    ...
    func lifetimeBoundsEnforced() {
        let tooShort = makeItem(lifetimeMonths: 0)
        let tooLong = makeItem(lifetimeMonths: 1000)
...
struct BackupTests {
    ...
    func roundTrips() throws {
        let item = makeItem(price: 1234.5, currency: "TWD", rate: 0.031, lifetimeMonths: 42)
